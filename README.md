...
### 筛选维度：类型归一与渠道归一（2026-09-22）

页面 **13 个枚举维度 + 关键词 + 状态页签**（2026-09-23 起是**四页签**结构：
数据总览 / 资费明细 / 变化历史 / 数据说明），完整对照表（取值、数据来源、四网是否同构）
见 `cloud/tariff/README.md`。这里只记**为什么必须归一**：

| | 移动 | 联通 | 电信 | 广电 |
|---|---|---|---|---|
| 上游原始分类 | 加装包 / 营销活动 / 套餐 / 标准资费 / 港澳台-国际 | ＋停售套餐 | 加装包 / 营销活动 / 套餐 | **5G套餐 / 促销 / 流量包 / 语音包 / 宽带 / 5G / 4G套餐 / 合约** |

广电那网**根本没有「套餐」这个词**（它写「5G套餐」「4G套餐」），联通还多一个「停售套餐」——
直接拿上游分类当筛选项，等于让用户按「上游怎么分的」去理解四个网。
所以在构建期按 `TYPE_CAT` 归一到大类（**套餐 / 加装包 / 营销活动 / 标准资费 / 宽带 / 港澳台-国际**），
页面只读不算。

> 映射表**列全**而不是靠关键字猜：「5G」这种名字里没写「套餐」的也得进套餐，
> 而按 `in "包"` 判断会把「套餐」本身也误吸进「加装包」。
> 漏映射的会落进「其他」—— CI 会警告（零星新分类不掐断当天的自动更新），
> 但占比超 1% 就硬失败（那说明映射整体失效）。

**渠道**同理：上游原文有几十种写法（移动光表达「线上」就有 8 种；
「自有渠道（中国联通APP等）、互联网渠道（飞猪、携程等）」看着像线下、实际纯线上），
归一到 线上 / 线下 / 线上+线下 / 其他，四网合计 **99.6%** 归得上，
剩下的是「不单独销售」这类根本没写渠道的描述 —— 一律落「其他」（不猜）。

**实测否决**：`tariffAttr`(1/2) 看着像「全国 / 本省目录」，但联通 `attr=1` 的 6197 条
地域**全在河北** ⇒ 它不等于适用范围，且各网语义不一致，做成筛选会与地域维度打架。

### 归属（个人 / 政企）：判据是 `type1`，不是「目标客户」文本

> 🔴 **2026-09-23 更正。** 本节原先的标题是「政企资费：四网公开渠道都取不到」，
> 并据此决定「不做政企层」—— **那个结论是错的。**
> 错在把「政企**门户**进不去」当成了「政企**资费**拿不到」，又拿条目
> 「目标客户」(`applicablePeople`) 文本去反推，只捞到 68 条，
> 于是把「文本里读不出来」误判成了「数据不存在」。
>
> 真相：**移动的政企资费一直就在抓取结果里** —— 上游的**条目级** `type1` 字段
> 就是「个人 / 政企」（`1`=个人、`2`=政企）；`type2` 才是「套餐 / 加装包 / 营销活动」。
> 河北移动有 **24 条**政企资费（样例「酒店悦享套餐-IPTV（15元档）」
> 「FTTO标准款（1主1从）-冀享专属」「企业组网套餐999元趸交版」），
> 只是 `rows_of()` 当时没把它写进行数据，页面上也就无从筛起。

各网实测（条目级）：

| 网 | 条目里有 `type1`？ | 政企条数 | 页面表现 |
|---|---|---|---|
| 移动 | ✅ 3887 条全有 | **24** | 「归属」下拉可用 |
| 联通 | ❌ 连键都不存在 | — | 下拉**隐藏** |
| 电信 | ❌ 连键都不存在 | — | 下拉**隐藏** |
| 广电 | ❌ 连键都不存在 | — | 下拉**隐藏** |

下面这些**仍然成立**，它们说的是「政企**门户 / 非公开入口**进不去」，
与本节的更正不冲突：

- 政企**门户**不公开：`qy.10010.com` = nginx 空壳（真实 Chrome 打开即「Welcome to nginx!」）、
  `qy.189.cn` = 403、`qy.10086.cn` = 内部运营后台；`qy.10099.com.cn` DNS 不存在；
- 移动 `isPublic="0"` 那套（非公开目录）**明细恒返 0 条**，旁路 path 回
  `40401 service not exist` ⇒ **下架数据**确实拿不到。这条只影响「已下架」页签，
  **不影响政企** —— 政企条目就在公开接口的 `type1=2` 里；
- 联通 / 电信 / 广电的上游**确实没有** `type1` 字段（不是我们没取到），
  所以「归属」维度在这三网**隐藏**，而不是显示一个永远筛出 0 条的假下拉。

> 🔴 **教训（已钉进 CI）**：「目标客户」是给人读的长文本，不是枚举字段 ——
> 「中国移动用户（不含港澳台）」这类写法在四网占 **84%~99%**，
> 拿它反推「个人 / 政企」等于从自然语言里猜分类，必然既漏又错。
> **归属只能来自上游结构化的枚举字段。**
> `tariff-daily.yml` 现在断言：移动政企必须 >0，其余三网必须一条 `ow` 都没有。

> 地市那条的**完整推导**（可达性矩阵、联通 4 条独立证据、广电 BASE102、
> 文本分档表）见 `docs/政企与地市-深挖结论-20260922.md`；
> 结论都能用 `probes/probe_city_matrix.py` / `probe_upstream_limits.py` / `probe_zq_text.py` 复跑。
> ⚠️ 那份文档写作时**尚未发现 `type1` 的语义**，其中「政企取不到」的部分已被本节取代。
