...
- **移动** —— `isPublic='0'` 会多列出 7~9 个分类组合，看着像「能拿到非公开/下架口径」，
  但明细接口 `getTariffListInfo` 在 `linkScn=1/2/3` 下**恒返回 `total=0, beans=0`**。
  参数能传 ≠ 拿得到数据。

### 地域口径与「已下架」

**地域**：每行按**原始地域字段**打一个 `sc` 标（`xz` 邢台 / `hb` 河北 / `cn` 全国），
页面据此筛选，与河北无关的（其他省份专属）条目直接剔除。四网判据**完全不同**
（`tariff_monitor.SCOPE_OF`），别想找一套通用规则：

| 网 | 邢台 | 河北 | 全国 | 判据 |
|---|---|---|---|---|
| 移动 | 104 | 2334 | 1449 | `applicableArea`/`city`/`province` **三个**字段联合判：`HE`=河北、`000`=全国、`3190`=邢台、多省 CSV=全国、`!XX`=排除式（等效全国） |
| 联通 | — | 7899 | — | 条目里**没有**地域字段 ⇒ 一律河北 |
| 电信 | 26 | 858 | — | `applicableArea`（`HE` / 地市码 CSV，含 `3190` 即邢台） |
| 广电 | — | 131 | 193 | `_areaNames`（只有「全国 / 河北省」两档，上游无地市粒度） |

「—」= 该网公开渠道**没有这一档**。页面上对应的筛选项会**置灰**，而不是让你选完
得到「0 条」—— 后者是拿一个不存在的维度在问，答案没有意义。

🔴 **别用名称文本猜地名**（初版做法）：名称没写地市、地市只写在「目标客户」里的条目
会整批漏判。改成按原始字段判之后，移动 3887 条**无一落入「无关」桶**。
