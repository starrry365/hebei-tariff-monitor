...
def fetch_all(include_stopped=False):
    ...
    return {"province": raw.get("province"), "provinceName": raw.get("provinceName"),
            ...
            # ★ 广电是唯一**真的没有地市粒度**的网：接口只有「全国 / 河北省」两档地区，
            #   条目里没有地市字段，页面下拉里选任何地市都只会得到 0 条。
            #   ⚠️ 这里不再用 allProvince 之类的**声明**去告诉页面「别出地市这一层」
            #   （联通那网的教训：声明错了，页面就跟着错，而且看不出）。
            #   现在页面按「本网有没有 `d.cty`」自己判 —— 广电一条 cty 都没有，
            #   地市那一层自然不出现，无需任何声明。
