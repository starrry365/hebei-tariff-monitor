...
★ 采**12 个地市的并集**（2026-09-24 修正，此前写的是「只采一城」）。
  ...
  **恰好全省一致**。全量实测：22 个 (一级×二级) 组合里 **12 个随城市变化**，
  12 个地市**每个都有专属条目**（雄安「雄安工地0元50G流量包」、沧州 41 个
  专属三级目录「华油专属」、保定「保定理工学院5G随行专网」…）；单城(邢台)三级目录 8991 个
...
def fetch_all(workers=6, city=None, include_stopped=True):
    ...
    return {"province": raw.get("province"), "provinceName": raw.get("provinceName"),
            ...
            # ★ 采集口径 = 12 城**并集**，且每条资费带回自己的地市归属
            #   （`_cityNames` / `_allCity`，见 he_unicom_tariff.collect）。
            #   🔴 这里**故意不再有 allProvince 声明**（2026-09-24 删）：
            #   那个字段当年写的是「本网数据不分城市」，而它**是错的** ——
            #   实测 22 个 (一级×二级) 组合里 12 个随 cityId 变化，12 个地市**每个都有**
            #   专属条目（雄安「雄安工地0元50G流量包」、沧州 **57 条资费**／41 个三级目录「华油专属」…），
            #   单城(邢台) 三级目录 8991 个 → 12 城并集 9121 个，**单城静默漏 130 个**。
            #   更糟的是它被页面当成「本网没有地市这一层」的依据，于是错得更彻底：
            #   一个**声明**与**数据**不一致时，页面信声明、不信数据。
            #   ⇒ 现在页面按 `d.cty` 有没有值来决定要不要出这一层（数据驱动），
            #     声明本身取消 —— 少一个能与现实脱节的旋钮。
