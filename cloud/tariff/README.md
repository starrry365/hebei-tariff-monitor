...
| `selftest_pipeline.py` | **集成缝自测**：网络换成桩，专测函数之间的接线（`other_nets` 四个分支 / 返回契约 / note 覆盖 / **四网注册表自洽**） | 否 |
| `check_nets_refactor.py [--from REV]` | **四网派生视图对拍**：8 个老表的值/类型/键顺序 + `where_of` 25 例 / `state_of` 14 例，与重构前逐项比。改了四网元信息（`NetSource`）后跑；`--from` 可现场比某个 git 版本 | 否 |
...
| `notify.py --test` | 真发一条自检消息（**会发出去**，只在本机验证通道时用） | 否 |
...
## 变更护栏与推送（2026-09-26 立）

上游按随机子集采样时，「本轮没采到」与「真下架」**形态完全一致** —— 唯一判据是上/下线日期。
所以「新增 / 下线 / 字段变更」这三个数字在进报告之前要过一层护栏（`change_guard.py`），
推送（`notify.py`）只在这层之后汇总成**一条**消息发出去。

🔴 全部判据遵循同一条原则：**判不出来就保留**。护栏要摘的是「明显是假的」那批，
不是「拿不准的」那批 —— 少报比多报严重得多（多报会被人追问，少报没人会知道）。

| 护栏 | 挡什么 | 开关（默认全开） |
|---|---|---|
| ① 逐条日期核验 | 假下架（下线日还没到）/ 假新增（上线日早于上一轮基线 → 归**补录**） | `REJECT_FUTURE_OFFLINE` / `REJECT_STALE_ONLINE` |
| ② 身份漂移 | 同一条资费换栏目被记成「下线一条 + 新增一条」（身份键含栏目名） | `DETECT_RELOCATE` |
| ③ 回弹检测 | 本轮新增正好是上一轮下线（增删互换）⇒ 基线回弹，丢弃不记 | `DETECT_REBOUND` |
| ④ 结构变更 | 字段结构变了 ⇒ 只重建基线、**不通知**（否则一次改动炸出上千条「字段变更」） | `REBUILD_ON_SCHEMA` |
| ⑤ 降级状态机 | 数据量偏低只低一轮就冻结旧数据，连续 `DEGRADE_ACCEPT_ROUNDS` 轮才接受 | `DEGRADE_RATIO` / `DEGRADE_ACCEPT_ROUNDS` |
| — 总开关 | `GUARD=0` 完全退回旧口径（故障时用来对比，**不是**日常开关） | `GUARD` |
| — 硬失败 | 降级时默认「页面照旧出 + `::warning`」；设 `DEGRADE_STRICT=1` 恢复「退出码 2」 | `DEGRADE_STRICT` |

⚠️ **停售桶是真实下架，不是漂移**：联通把「停售」编码成一级分类 `99`。
同编号从在售目录挪进停售目录 = **真下架**（实测 110 条），且该桶 `endDate` 几乎都未到期
（实测 4/110 写着 `2029-12-31`）⇒ 拿日期复核会把真下架全判成假下架，所以这类移动**豁免日期判据**。

推送两个通道：**邮件（SMTP）** 与 **PushPlus（微信）**。凭据全部走 Actions Secrets，
**一个都没配就静默跳过**（本地跑 / fork 别人仓库不会因此变红）。完整配置见
[`docs/变更护栏与推送配置-20260926.md`](../../docs/变更护栏与推送配置-20260926.md)。

🔴 推送的两条硬约束：
- **一次变化只推一条** —— 消息在 `main()` 末尾统一汇总；`.notify_state.json` 记内容指纹去重
  （CI 每轮全新 checkout，只在内存去重等于没去重）。
- **零变化不推**（`NOTIFY_ALWAYS=0`）—— 但**异常态必推**：降级 / 结构变更 / 采集失败
  在正文里与「无变化」长得很像，却是**需要人看一眼**的时刻。

## 四网注册表（2026-09-26 立）

四网（移动 / 联通 / 电信 / 广电）在 `tariff_monitor.py` 里各是一个 `NetSource` 子类实例，
`NETS` 是注册表本体。**一网的「全部知识」收在一个对象里**：显示名、快照前缀、适配器模块、
报告文件名前缀、支不支持停售、进不进本地缓存、采不到时兜底哪份快照、地域判据、下架判据。

重构前，这些属性散在 **8 张平行表 + 3 处 `if code ==` 分叉**里 —— 接一条新网要同时改 8~10 处，
而漏一处的症状各不相同、**都不报错**：

| 漏了哪个 | 症状（一律不报错） |
|---|---|
| `NET_SNAP` | 该网采不到时页面直接少一网 |
| `SNAP_PREFIX` | 它的快照混进别网的 `load_prev` 排序 ⇒ 拿别网数据当自己的基准，**数字全错且看不出原因** |
| `NET_RUN` | 该网永远不被采集（页面显示 0 条，看着像上游没数据） |
| `NET_STOPPED` | 该网的「已下架」整批不采（页签空了） |

**加一条新网 = 加一个 `NetSource` 子类 + 注册一行**，剩下的由自检盯住（见上表两行）。

🔴 **两条边界，别越**：

1. **8 张老表全部保留**，改成由注册表**派生**（`NETS_META` / `SRC_OF` / `NET_LIVE` / `NET_SNAP` /
   `SNAP_PREFIX` / `NET_RUN` / `NET_STOPPED` / `NET_NOCACHE`）。外部有 4 个脚本在读它们：
   ...
   **删掉任何一个都是静默断链** —— 那些脚本不报错，只会少采 / 采错。
2. **本层不搬采集代码**：移动的直连采集留在本文件，另三网的归一化层仍是 `X_monitor.py` ——
   它们各被 `rebuild_offline.py` 与 `probes/` 下的探针直接 `__import__` 引用。
   本层管的是**注册与派发**，不是把采集实现收进来。

⚠️ **顺序也是接口的一部分**：`NETS_META`（页面页签）与 `NET_LIVE`（采集顺序 + 顶部提示语拼接）
在 telecom/cbn 上顺序**相反**，`WHERE_OF` 又是第三种。全是历史遗留，**别为了整齐合并** ——
它们决定页面上「· 联通新增 1 / 变更 0 · 广电无变化 · 电信沿用 …」的先后。

这是**纯重构（行为零变化）**，证据是 `check_nets_refactor.py` 的逐项对拍 ——
8 个派生视图的值 / 类型 / 键顺序 + `where_of` 25 例 / `state_of` 14 例，与重构前**完全一致**；
首轮对拍就在这里抓到一处我自己没预料到的差异（`WHERE_OF` 的键顺序）。
